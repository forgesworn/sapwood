...
test('protected phone handoff shows a dedicated remote connection state', async ({ page }) => {
  ...
  await expect(state).toContainText('Opening secure relay connection')
  await expect(state).toContainText('Signed status request sent to relays')
  await expect(state).toContainText('Waiting for the signer to answer')
  await expect(state).toContainText('Signer response authenticated')
  await expect(state).not.toContainText('relay.invalid')
  await expect(state).not.toContainText(deviceHex)
  await expect(state).not.toContainText(opHex)
